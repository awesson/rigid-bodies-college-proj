...
BoxMesh::BoxMesh()
{
    ...
    // BoxMesh_front does not do this for speed purposes
    for(int i = 0; i < 2; ++i)
        subdivide();
...
void BoxMesh::subdivide(){
	...
	int prev_num_vertices = vertices.size();
    vertices.resize(prev_num_vertices + edges.size());
    new_triangles.resize(4 * triangles.size());

    // add new vertices
    for(int i = 0; i < edges.size(); ++i){
        vertices[prev_num_vertices + i].position = (vertices[edges[i].vertices[0]].position + vertices[edges[i].vertices[1]].position)/2.0;
        vertices[prev_num_vertices + i].normal = vertices[edges[i].vertices[0]].normal + vertices[edges[i].vertices[1]].normal;
        unitize(vertices[prev_num_vertices + i].normal);
        edges[i].new_v = prev_num_vertices + i;
    }
